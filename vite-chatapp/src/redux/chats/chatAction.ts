...
// GET CHATS 
export const getChats =(endpoint: string) =>
...
export const createGroup =(groupname:string, users:string) =>
  async (dispatch: Dispatch<GetChatsActionTypes>): Promise<void> => { 
...
export const sendMessages =(endpoint: string, content: string, chatId: string) =>
  async (dispatch: Dispatch<GetChatsActionTypes>): Promise<void> => {
    ...
    try {
      ...
      // console.log(res.data);
      dispatch({ type: SEND_MESSAGES_SUCCESS, payload: res });
    } catch (error) {
      dispatch({ type: SEND_MESSAGES_ERROR });
    }
  };

  
// getting all messages
export const getMessages =(endpoint: string, chatId: string) =>
  async (dispatch: Dispatch<GetChatsActionTypes>): Promise<void> => {
